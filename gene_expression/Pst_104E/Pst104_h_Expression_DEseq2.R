...
library(pheatmap)
########################################################################################
setwd("H:/Work_Csiro/Pst-104e_Analysis")
##########################################################
# Now try DESeq2 on the hcontig data
##########################################################
countsTable = read.delim(file = "Pst_104E_v12_h_ctg.in_house.STAR.featureCounts.csv", 
                         sep=",", header=TRUE, 
                         row.names=1)
head(countsTable)
...
write.table(assay(rld), file="Pst104_h_ctg_rlogsFull.txt", sep="\t")
...
tiff("DeSeq2_PCAplot_Pst104_h_ctgs.tiff", height = 6, width = 8, units = 'in', res = 300)
g1
dev.off()
########################################################################################
...
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_HaustoriavsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_6dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_9dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_UngerminatedSporesvsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_9dpivs6dpi.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_6dpivsHaustoria.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_hctg_9dpivsHaustoria.csv")
########################################################################################
...
png("Pst104_h_Top500GenesExpressed_Clustering_kmeans.png", height = 30, width = 15, units = 'in', res = 300)
