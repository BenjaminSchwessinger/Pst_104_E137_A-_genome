...
##########################################################
countsTable = read.delim(file = "Pst_104E_v12_p_ctg.in_house_IDParent.STAR.featureCounts.csv", 
                         sep=",", header=TRUE, 
                         row.names=1)
head(countsTable)
...
tiff("DeSeq2_PCAplot_Pst104_p_ctgs.tiff", height = 6, width = 8, units = 'in', res = 300)
g1
dev.off()
########################################################################################
...
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_HaustoriavsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_6dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_9dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_UngerminatedSporesvsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_9dpivs6dpi.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_6dpivsHaustoria.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_pctg_9dpivsHaustoria.csv")
########################################################################################
...
png("Pst104_Top500GenesExpressed_Clustering_kmeans.png", height = 30, width = 15, units = 'in', res = 300)
