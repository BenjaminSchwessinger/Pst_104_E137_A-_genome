...
########################################################################################
##########################################################
# Now try DESeq2 on the SD80 data
##########################################################
countsTable = read.delim(file = "counts-SD80-all.csv", 
                         sep=",", header=TRUE, 
                         row.names=1)
head(countsTable)

coldata <- read.csv("DeSeq2_SD80Expression.csv",row.names=1)
head(coldata)
...
tiff("DeSeq2_PCAplot_SD80_ctgs.tiff", height = 6, width = 8, units = 'in', res = 300)
g1
dev.off()
########################################################################################
...
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_HaustoriavsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_2dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_5dpivsGerminatedSpores.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_5dpivs2dpi.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_2dpivsHaustoria.csv")
########################################################################################
...
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_SD80_5dpivsHaustoria.csv")
########################################################################################
...
png("SD80_Top500GenesExpressed_Clustering_kmeans.png", height = 30, width = 15, units = 'in', res = 300)
