...
########################################################################################
##########################################################
# Now try DESeq2 on the NC29 data
##########################################################
countsTable = read.delim(file = "counts-NC29-all.csv", 
                         sep=",", header=TRUE, 
                         row.names=1)
head(countsTable)

coldata <- read.csv("DeSeq2_NC29Expression.csv",row.names=1)
head(coldata)
...
tiff("DeSeq2_PCAplot_NC29_ctgs.tiff", height = 6, width = 8, units = 'in', res = 300)
g1
dev.off()
########################################################################################
...
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_HaustoriavsGerminatedSpores.csv")
########################################################################################
res <- results(dds, contrast=c("condition","2dpi","GerminatedSpores"))
res
summary(res)
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_2dpivsGerminatedSpores.csv")
########################################################################################
res <- results(dds, contrast=c("condition","5dpi","GerminatedSpores"))
res
summary(res)
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_5dpivsGerminatedSpores.csv")
########################################################################################
res <- results(dds, contrast=c("condition","5dpi","2dpi"))
res
summary(res)
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_5dpivs2dpi.csv")
########################################################################################
res <- results(dds, contrast=c("condition","2dpi","Haustoria"))
res
summary(res)
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_2dpivsHaustoria.csv")
########################################################################################
res <- results(dds, contrast=c("condition","5dpi","Haustoria"))
res
summary(res)
sum(res$padj < 0.1, na.rm=TRUE)
write.csv(as.data.frame(res), 
          file="DEseq2_NC29_5dpivsHaustoria.csv")
########################################################################################
...
png("NC29_Top500GenesExpressed_Clustering_kmeans.png", height = 30, width = 15, units = 'in', res = 300)
heatmap.2(m.kmeans[,1:12], col = hmcol, trace="none", margin=c(10, 30), 
          Colv=FALSE, Rowv=FALSE, density.info='none', lhei=c(0.03, 1), 
          RowSideColors=as.character(as.numeric(m.kmeans[,13])),
